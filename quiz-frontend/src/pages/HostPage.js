...
export default function HostPage() {
  ...
  if (screen === STATES.WAITING) return (
    <HostShell>
      <HostGlow top="10%" left="12%" size={340} color="rgba(95, 151, 255, 0.16)" />
      <HostGlow top="72%" left="72%" size={360} color="rgba(255, 153, 0, 0.12)" />
      <HostStars />

      <HostStage style={{ maxWidth: 940, padding: '44px 40px 40px' }}>
        <HostPill>AWS Cloud Club Quiz</HostPill>
        <HostTitle style={{ marginTop: 22 }}>Katılmak için kodu gir</HostTitle>
        <HostSubtle>Bu kodla da doğrudan yarışmaya katılabilirsiniz.</HostSubtle>
        ...
        <div style={{ marginTop: 26, display: 'flex', justifyContent: 'center' }}>
          <div style={{
            ...
            height: 168,
            borderRadius: 28,
            background: 'linear-gradient(180deg, rgba(255,255,255,0.96), rgba(232, 238, 255, 0.9))',
            display: 'flex',
            alignItems: 'center',
  ...
  if (screen === STATES.COUNTDOWN) return (
    <HostShell>
      <HostGlow top="16%" left="18%" size={320} color="rgba(95, 151, 255, 0.16)" />
      <HostGlow top="58%" left="62%" size={340} color="rgba(255, 153, 0, 0.14)" />
      <HostStage style={{ maxWidth: 720, padding: '50px 30px 56px' }}>
        <HostPill>Başlangıç Ekranı</HostPill>
        <HostTitle style={{ marginTop: 24, fontSize: 64 }}>Oyun başlıyor</HostTitle>
        <HostSubtle style={{ maxWidth: 440, marginLeft: 'auto', marginRight: 'auto' }}>İlk soru birazdan başlıyacak. Oyuncular hazır.</HostSubtle>
        <div style={{
          marginTop: 28,
          marginLeft: 'auto',
          marginRight: 'auto',
          ...
          <div style={{
            ...
            display: 'grid',
            placeItems: 'center',
            background: 'linear-gradient(180deg, rgba(255,153,0,0.36), rgba(255,255,255,0.06))',
            color: '#fff',
            fontSize: 138,
            fontWeight: 900,
            lineHeight: 1,
  ...
  if (screen === STATES.QUESTION && question) return (
    <HostShell>
      <HostGlow top="10%" left="12%" size={320} color="rgba(95, 151, 255, 0.14)" />
      <HostGlow top="70%" left="76%" size={340} color="rgba(255, 153, 0, 0.12)" />
      <HostStars />

      <HostStage style={{ maxWidth: 980, padding: '26px 24px 28px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 14 }}>
          <HostMetaPill>Soru {question.index + 1}/{question.total}</HostMetaPill>
          <HostTimerPill tone={timeLeft <= 5 ? 'danger' : timeLeft <= 10 ? 'warn' : 'normal'}>
            ⏱ {timeLeft}s
          </HostTimerPill>
          <HostMetaPill>{answerCount} cevap alındı</HostMetaPill>
        </div>

        <div style={{ width: '100%', height: 12, borderRadius: 999, background: 'rgba(255,255,255,0.1)', overflow: 'hidden', marginBottom: 24 }}>
          <div style={{
            width: `${(timeLeft / question.timerSeconds) * 100}%`,
            height: '100%',
            borderRadius: 999,
            background: timeLeft <= 5 ? '#FF6B6B' : timeLeft <= 10 ? '#FFB443' : '#48A7FF',
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: 'radial-gradient(circle, rgba(117, 194, 255, 0.16) 0%, rgba(0,0,0,0) 70%)',
            pointerEvents: 'none',
          }} />
          <h2 style={{
            position: 'relative',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, width: '100%' }}>
          {Object.entries(question.options).map(([k, v]) => (
            ...
            >
              <div style={{ fontSize: 30, fontWeight: 900, letterSpacing: '-0.03em', marginBottom: 8 }}>{k}</div>
              <div style={{ fontSize: 24, lineHeight: 1.2, fontWeight: 700 }}>{v}</div>
            </div>
          ))}
        </div>
      </HostStage>
    ...
    <HostShell justify="center" padTop={24}>
      <HostStage style={{ maxWidth: 920, padding: '34px 34px 36px' }}>
        <div style={{ textAlign: 'center', marginBottom: 26 }}>
          <HostPill style={{ background: 'rgba(255,255,255,0.08)' }}>Tur Sonucu</HostPill>
          <HostTitle style={{ marginTop: 18, fontSize: 54 }}>Süre doldu</HostTitle>
          <HostSubtle>
            Doğru cevap:
            <span style={{ color: optionColors[questionEnd.correctAnswer], fontSize: 54, fontWeight: 900, marginLeft: 16, verticalAlign: 'middle' }}>
        ...
        {['A','B','C','D'].map(k => {
          ...
          return (
            <div key={k} style={{ display: 'grid', gridTemplateColumns: '44px 1fr 76px', alignItems: 'center', marginBottom: 16, gap: 16 }}>
              <span style={{ width: 44, fontSize: 30, fontWeight: 'bold', color: optionColors[k], textAlign: 'center' }}>{k}</span>
              <div style={{ flex: 1, background: 'rgba(255,255,255,0.08)', borderRadius: 14, height: 52, overflow: 'hidden', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.06)' }}>
                <div style={{ background: k === questionEnd.correctAnswer ? '#69d37a' : optionBg[k], width: `${(count / max) * 100}%`, height: '100%', borderRadius: 14, display: 'flex', alignItems: 'center', paddingLeft: 16, transition: 'width 0.5s', minWidth: count > 0 ? 52 : 0 }}>
                  {count > 0 && <span style={{ fontSize: 22, fontWeight: 'bold' }}>{count}</span>}
                </div>
              </div>
              <span style={{ fontSize: 22, width: 76, textAlign: 'right', color: 'rgba(234,240,255,0.86)', fontWeight: 700 }}>{count} kişi</span>
            </div>
          );
        })}
        </div>
  ...
  if (screen === STATES.SCORE_REVEAL && scoreReveal) return (
    <HostShell>
      <HostGlow top="10%" left="12%" size={320} color="rgba(95, 151, 255, 0.14)" />
      <HostGlow top="72%" left="76%" size={340} color="rgba(255, 153, 0, 0.12)" />
      <HostStars />
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center', marginBottom: 16 }}>
        <h2 style={{ fontSize: 38, margin: 0, fontWeight: 900 }}>🏆 Sıralama</h2>
        {scoreReveal.nextQuestionAt > 0 && (
          <HostTimerPill tone="normal">Sonraki soru: {nextQuestionCountdown}s</HostTimerPill>
        )}
      </div>
      <div style={{ width: '100%' }}>
        {scoreReveal.top10.length === 0
          ...
          : scoreReveal.top10.map((p, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', marginBottom: 8, borderRadius: 16, fontSize: 22, background: i < 3 ? 'rgba(255,255,255,0.16)' : 'rgba(255,255,255,0.08)', color: '#fff' }}>
  ...
  if (screen === STATES.FINISHED) return (
    <HostShell>
      <HostGlow top="12%" left="14%" size={320} color="rgba(95, 151, 255, 0.14)" />
      <HostGlow top="72%" left="76%" size={340} color="rgba(255, 153, 0, 0.12)" />
      <HostStars />

      <div style={{ width: '100%', maxWidth: 700, position: 'relative', paddingTop: 106 }}>
        ...
        <HostStage style={{ maxWidth: 700, padding: '78px 34px 34px', textAlign: 'center' }}>
        <HostPill style={{ background: 'rgba(255,255,255,0.08)' }}>Final</HostPill>
        <HostTitle style={{ marginTop: 18, fontSize: 68 }}>Oyun bitti</HostTitle>
        <HostSubtle style={{ maxWidth: 420, marginLeft: 'auto', marginRight: 'auto' }}>
          Final liderlik tablosu aşağıda.
        </HostSubtle>

        <div style={{ width: '100%', maxWidth: 560, margin: '28px auto 0', display: 'grid', gap: 10 }}>
          {(gameFinished || []).map((p, i) => (
            <div
              ...
              style={{
                ...
                borderRadius: 18,
                padding: '16px 18px',
                background: i < 3 ? 'rgba(255,255,255,0.16)' : 'rgba(255,255,255,0.08)',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                <span style={{ width: 28, textAlign: 'center' }}>{['🥇','🥈','🥉','4.','5.'][i] || `${i + 1}.`}</span>
                <span style={{ fontSize: 20, fontWeight: 800 }}>{p.nickname || p.userId}</span>
              </div>
...
function HostStage({ children, style }) {
  return (
    <div style={{
      ...
      width: '100%',
      borderRadius: 34,
      background: 'linear-gradient(180deg, rgba(18, 10, 49, 0.9), rgba(43, 18, 78, 0.86))',
...
function HostTitle({ children, style }) {
  return <h1 style={{ margin: 0, fontSize: 72, lineHeight: 0.95, letterSpacing: '-0.05em', fontWeight: 900, textAlign: 'center', ...style }}>{children}</h1>;
}

function HostSubtle({ children, style }) {
  return <p style={{ margin: '14px 0 0', textAlign: 'center', color: 'rgba(232,239,255,0.76)', fontSize: 24, lineHeight: 1.45, fontWeight: 600, ...style }}>{children}</p>;
}
...
function HostMetricCard({ value, label, accent = '#eef4ff' }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 42, lineHeight: 1, fontWeight: 900, color: accent }}>{value}</div>
      <div style={{ marginTop: 10, color: 'rgba(222,230,255,0.58)', fontSize: 13, fontWeight: 900, letterSpacing: '0.16em', textTransform: 'uppercase' }}>{label}</div>
    </div>
  );
}
