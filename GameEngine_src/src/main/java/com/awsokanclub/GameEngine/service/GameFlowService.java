...
public class GameFlowService {
    ...
    @Value("${app.game.next-question-countdown-seconds}")
    private int nextQuestionDelaySeconds;

    @Value("${app.game.leaderboard-approval-timeout-ms}")
    private long leaderboardApprovalTimeoutMs;
    ...
    private void publishLeaderboardPending(String gameId, String questionId) {
        ...
        // Sadece personal queue — host topic'e de göndermek çift mesaja yol açıyor.
        // Admin hem /user/queue/admin hem /topic/game/.../host'a subscribe olduğundan
        // broadcastToHost eklenince LEADERBOARD_PENDING iki kez geliyor.
    ...
    public void publishScoreReveal(String gameId, String questionId) {
        // Atomic Redis lock — timer ve admin approve aynı anda geldiğinde ikisi birden çalışmasın
        String lockKey = "lock:scorereveal:" + gameId + ":" + questionId;
        Boolean locked = redisTemplate.opsForValue().setIfAbsent(lockKey, "1", 60, TimeUnit.SECONDS);
        if (!Boolean.TRUE.equals(locked)) {
            log.warn("publishScoreReveal: zaten çalışıyor, atlanıyor: gameId={} q={}", gameId, questionId);
            return;
        }
        ...
        long nextQuestionAt = hasNext
                ? System.currentTimeMillis() + (nextQuestionDelaySeconds * 1000L)
                : 0;
        ...
        if (hasNext) {
            ...
            taskScheduler.schedule(
                    ...
                    Instant.now().plusMillis(nextQuestionDelaySeconds * 1000L));
        } else {
            finishGame(gameId);
        }
    }
    ...
    public void finishGame(String gameId) {
        ...
        log.info("Oyun bitti ve temizlendi: gameId={}", gameId);
    }

    // ── Redis'i tamamen temizle ────────────────────────────
    // Her oyun kendi sessionCode'uyla izole — bitince tüm keyler silinir.
    // Bir sonraki oyun yeni sessionCode alacağından stale state imkansız.
    private void cleanupGameRedis(String gameId, List<GameSession> sessions) {
        try {
            // Session key'lerini tek tek sil (players set silinmeden önce)
            for (GameSession s : sessions) {
                redisTemplate.delete("session:" + s.getSessionId());
                redisTemplate.delete("user_to_session:" + gameId + ":" + s.getUserId());
            }

            // Bilinen sabit key'leri sil
            ...
            log.error("Redis temizleme hatasi: gameId={} hata={}", gameId, e.getMessage());
        }
    }
    ...
    private void deleteByPattern(String pattern) {
        try {
            ...
            log.warn("Pattern silme hatasi: pattern={} hata={}", pattern, e.getMessage());
        }
    }
    ...
    private void saveResultsToGameAdmin(String gameId) {
        try {
            ...
            for (GameSession session : sessions) {
                int score = leaderboardService.getUserScore(gameId, session.getUserId());
                // Rank: leaderboard'da yoksa (0 puan) toplam oyuncu sayısına eşit rank ver
                int rank = rankMap.getOrDefault(session.getUserId(), totalPlayers);
